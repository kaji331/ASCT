...
@inline function ReducedNet(network::NetWork,
    ...
    @inbounds for i in 1:clustering.clusters_number
        j = Ref{Int}(1)
        @simd for k in 1:length(nodes_per_cluster[i])
            l = nodes_per_cluster[i][k]
            node_weights[i] += network.node_weights[l]
            Threads.@threads for m in (network.first_neighbor_ind[l]:
...
function NodesNumberPerCluster(clustering::MClustering)
    ...
    @inbounds for clust in clustering.clusters
...
function NodesPerCluster(clustering::MClustering)
    # Hard, but fast
    nodes_per_cluster = [ Int[] for i in 1:clustering.clusters_number ]
    ...
    @inbounds for i in 1:clustering.nodes_number
...
@inline function Quality(vosct::VOSct)
    ...
    @inbounds for i in 1:vosct.network.nodes_number
        j = vosct.clustering.clusters[i]
        Threads.@threads for k in (vosct.network.first_neighbor_ind[i]:
    ...
    @inbounds for i in 1:vosct.clustering.clusters_number
        quality_fun[] -= cluster_weights[][i] ^ 2  * vosct.resolution
    end
...
@inline function LocalMoving!(vosct::VOSct,
    ...
    @inbounds for i in 1:vosct.network.nodes_number
        if nodes_number_per_cluster[i] == 0
            unused_clusters[unused_clusters_number + 1] = i
            unused_clusters_number += 1
        end
    end
    ...
    while true
        ...
        Threads.@threads for k in (vosct.network.first_neighbor_ind[j]:
        ...
        if nodes_number_per_cluster[vcc[][j]] == 0
            unused_clusters[unused_clusters_number + 1] = vcc[][j]
            unused_clusters_number += 1
        end
        best_cluster = -1
        max_quality_fun = 0
        @inbounds for k in 1:neighboring_clusters_number[]
            l = neighboring_clusters[][k]
            quality_fun = edge_weights_per_cluster[][l] - vnnw[][j] * 
        ...
        if max_quality_fun == 0
            best_cluster = unused_clusters[unused_clusters_number]
            unused_clusters_number -= 1
        end
...
function MatrixToNetwork(node1::Vector{Int},
    ...
    n1_max = maximum(node1)
    n2_max = maximum(node2)

    neighbors_number = zeros(Int,nodes_number)
    @inbounds for i in eachindex(node1)
        if node1[i] < node2[i]
    ...
    edges_number = 0
    @inbounds for i in 1:nodes_number
